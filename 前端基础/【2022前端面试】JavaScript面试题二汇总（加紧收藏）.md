# 【2022前端面试】JavaScript面试题二汇总（加紧收藏）
...
- 首先有一点在JavaScript中是使用`构造函数`来新建一个对象的。
- 当使用构造函数新建一个对象后，在这个对象的内部将包含一个指针，这个指针（`__proto__`）指向构造函数的 `prototype` 属性对应的值，在 ES5 中这个指针被称为**对象的原型**。
- ES5 中新增了一个 `Object.getPrototypeOf() `方法，可以通过这个方法来获取对象的原型。
- **原型链**中就是实例对象和原型对象之间的链接。
